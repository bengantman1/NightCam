...
/* -------------------------------------------------------------------------
 * GET /  →  HTML page listing all clip directories
 * -------------------------------------------------------------------------*/
static esp_err_t handle_root(httpd_req_t *req)
{
    ...
    if (!root) {
        ...
        while ((entry = readdir(root)) != NULL) {
            ...
            // Send static HTML and variable name separately — no mixed snprintf
...
static esp_err_t handle_clip(httpd_req_t *req)
{
    ...
    send_str(req, 
        ...
        "</div>"
        "<ul>" // Now start the list of files
    );  
    ...
    // --- JAVASCRIPT REMAINS HERE (BOTTOM OF PAGE) ---
    send_str(req, 
        "<script>"
...
{
    // 1. Safely extract path and strip query params/fragments
    const char *rel_ptr = req->uri + strlen("/file/");
    char rel[64];
    
    // Copy to a local buffer so we can sanitize it
    ...
    // --- FIX APPLIED HERE ---
    // Content-Length calculation is removed. httpd_resp_send_chunk 
    // forces chunked encoding; supplying a Content-Length conflicts 
    // with this and corrupts the download.
...
void wifi_server_task(void *pv)
{
    ...
    while (1) {
        ...
        xEventGroupWaitBits(event_group, WIFI_ACTIVE, 
                            pdFALSE,  // Do NOT clear yet; we clear on shutdown
                            pdTRUE, 
                            portMAX_DELAY);
