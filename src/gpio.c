...
void IRAM_ATTR pir_isr(void* arg) {
    ...
    if (!(state & (CAMERA_ACTIVE | WIFI_ACTIVE))) {
        int64_t now = esp_timer_get_time();
        // software debouncing
        if (now - last_pir_isr_time > DEBOUNCE_DELAY_US) {
            last_pir_isr_time = now;
            // Notify task that does the work (start recording)
            xEventGroupSetBitsFromISR(event_group, CAMERA_ACTIVE, &xHigherPriorityTaskWoken);
        }
    }
}
...
void IRAM_ATTR btn_isr(void* arg) {
    EventBits_t state = xEventGroupGetBitsFromISR(event_group);
    BaseType_t xHigherPriorityTaskWoken = pdFALSE;
    // only activate if not recording, and WIFI off
    if (!(state & (CAMERA_ACTIVE | WIFI_ACTIVE))) {
        int64_t now = esp_timer_get_time();
        // software button debouncing
        if (now - last_btn_isr_time > DEBOUNCE_DELAY_US) {
            last_btn_isr_time = now;
            // Notify wifi task to begin access point
...
void gpio_init_all() {
    ...
    event_group = xEventGroupCreate(); // 2/24 event bits used
    ...
    gpio_config(&pir);
    // Install ISR service for all interrupts
